...
public class MazeGenerator
{
    ...
    private void GeneratePerfectMaze()
    {
        ...
        if (y - 2 >= 1) toCheck.Add(new Point(x, y - 2));
        if (y + 2 < height - 1) toCheck.Add(new Point(x, y + 2));
        if (x - 2 >= 1) toCheck.Add(new Point(x - 2, y));
        if (x + 2 < width - 1) toCheck.Add(new Point(x + 2, y));
...
public class Program
{
    ...
    {
        // Создаем генератор лабиринта размером 21x21
        ...
        // Показываем координаты входа и выхода
        var start = generator.GetStartPoint();
        var end = generator.GetEndPoint();
        Console.WriteLine($"\nВход: ({start?.X}, {start?.Y})");
        Console.WriteLine($"Выход: ({end?.X}, {end?.Y})");
